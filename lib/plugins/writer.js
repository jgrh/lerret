...
function createRootFileStream(name) {
    return fs.createOutputStream(getRootFilename(name));
}
...
function createAlbumFileStream(album, name) {
    return fs.createOutputStream(getAlbumFilename(album, name));
}
...
function createImageFileStream(album, image, name) {
    return fs.createOutputStream(getImageFilename(album, image, name));
}
