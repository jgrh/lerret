...
async function loadProjectPlugins() {
    ...
    try {
        ...
        filenames = await Promise.map(fs.readdirAsync(pluginDirectory), filename => path.resolve(pluginDirectory, filename));
    }
    catch (e) {
        if (e.code == "ENOENT") {
            return;
    ...
    const pluginFilenames = await Promise.filter(filenames, async (filename) => {
        try {
            return !path.basename(filename).startsWith(".") && ((await fs.statAsync(filename)).isDirectory() || path.extname(filename) === ".js");
        }
        catch (e) {
