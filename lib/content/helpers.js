...
async function listSubdirectories(directory) {
    let filenames;
    try {
        filenames = await Promise.map(fs.readdirAsync(directory), filename => path.join(directory, filename));
    }
    catch (e) {
    ...
    return Promise.filter(filenames, async (filename) => {
        try {
            return !filename.startsWith(".") && (await fs.statAsync(filename)).isDirectory();
        }
        catch (e) {
...
async function readYaml(filename) {
    try {
        const fd = await fs.readFileAsync(filename);
